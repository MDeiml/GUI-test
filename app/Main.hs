...
test1 :: App t
test1 = proc _ -> do
    widgetOutput  -< ((stdParams { pHeight = 150, pWidth = 100 }, False), \b -> [DrawShape (Color 255 0 0) $ Rect b])
    widgetOutput  -< ((stdParams { pWeightY = Just 1, pWeightX = Just 1 }, False), \b -> [DrawShape (Color 0 255 0) $ Rect b])
    widgetOutput  -< ((stdParams { pWeightY = Just 2, pWeightX = Just 1 }, False), \b -> [DrawShape (Color 0 0 255) $ Rect b])
    widgetOutput  -< ((stdParams { pHeight = 50, pWidth = 40 }, False), \b -> [DrawShape (Color 255 255 255) $ Rect b])
...
focusWidget = stackLayout (0,0,0,0) (AlignCenter, AlignCenter) (Just 1, Just 1) $ proc _ -> do
    ...
    widgetOutput -< ((stdParams {pWeightX = Just 1, pWeightY = Just 1}, False), \b -> [DrawShape c $ Rect b])
    returnA -< f
...
test4 = stackLayout (0, 0, 0, 0) (AlignCenter, AlignCenter) (Just 1, Just 1) $ proc _ -> do
    ...
    widgetOutput -< ((stdParams, False), (:[]) . DrawShape (Color 255 0 0) . Rect)
